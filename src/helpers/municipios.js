const MUNICIPIOS = [
  {
    provincia_id: 1,
    municipio_id: 101,
    municipio: "SANTO DOMINGO CENTRO (DN)",
  },
  {
    provincia_id: 71,
    municipio_id: 201,
...
const getMunicipiosByProvincia = (provinciaId) => {
  const results = MUNICIPIOS.filter(
    ({ provincia_id }) => provincia_id == provinciaId
  );
  return results;
};
const getMunicipioById = (municipioId) => {
  const result = MUNICIPIOS.find(
    ({ municipio_id }) => municipio_id == municipioId
  );
  return result;
};
const getMunicipioByIds = (municipioId, provinciaId) => {
  const result = MUNICIPIOS.filter(
    ({ provincia_id }) => provincia_id == provinciaId
  ).find(({ municipio_id }) => municipio_id == municipioId);
  return result;
};
const getMunicipios = () => {
  return MUNICIPIOS;
